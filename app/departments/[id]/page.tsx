...
export default function DepartmentPage() {
  ...
  const editMemberName = async (member: TeamMember) => {
    const nextName = window.prompt('Edit member name', member.name)?.trim()
    if (!nextName || nextName === member.name) return
            ...
            <div className="rounded-lg border bg-white p-4">
              ...
              <div className="space-y-1">
                {members.map((m) => (
                  <div
                    key={m.id}
                    className={`w-full px-3 py-2 rounded text-sm border flex items-center justify-between ${
